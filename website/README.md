...
- 카테고리 필터, 상품 미리보기, 다국어 카테고리 검색
- 한국어·영어·일본어·중국어(간체) 전환, 선택 언어 저장, `?lang=en`·`?lang=ja`·`?lang=zh-CN` 공유 링크
- 컬렉션, 브랜드 스토리, FAQ, 공식몰 연결
- 움직임 줄이기 시스템 설정, 키보드 탐색, 모달 Escape 닫기·초점 복원

가격, 재고, 사이즈, 구매 및 계정 기능은 떼뚜 공식몰로 연결됩니다. 언어 전환은 이 소개 사이트에 적용되며 외부 공식몰의 언어는 변경하지 않습니다. 로컬에서 주문·결제·로그인 정보를 처리하지 않습니다. 히어로는 AI 콘셉트 이미지로 실제 판매 상품을 나타내지 않습니다. 카테고리 이미지와 캠페인 이미지는 공개된 떼뚜 공식몰 자료입니다.
